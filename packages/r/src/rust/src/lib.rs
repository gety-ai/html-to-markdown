...
impl NodeContent {
    ...
    pub fn _factory_image(description: Option<String>, src: Option<String>, image_index: Option<i32>) -> NodeContent {
...
pub fn convert(html: String, options: Robj) -> Result<Robj> {
    ...
    let visitor_handle: Option<html_to_markdown_rs::VisitorHandle> =
        visitor_robj.map(|v| Arc::new(Mutex::new(RHtmlVisitorBridge::new(v))) as html_to_markdown_rs::VisitorHandle);
...
fn nodecontext_to_robj(ctx: &html_to_markdown_rs::NodeContext) -> extendr_api::Robj {
    use extendr_api::prelude::*;
    let mut pairs: Vec<(&str, extendr_api::Robj)> = Vec::new();
    pairs.push((
        "node_type",
        extendr_api::Robj::from(format!("{:?}", ctx.node_type).as_str()),
    ));
    pairs.push(("tag_name", extendr_api::Robj::from(AsRef::<str>::as_ref(&ctx.tag_name))));
    pairs.push(("depth", extendr_api::Robj::from(ctx.depth as f64)));
    pairs.push(("index_in_parent", extendr_api::Robj::from(ctx.index_in_parent as f64)));
    pairs.push((
        "parent_tag",
    ...
    pairs.push(("is_inline", extendr_api::Robj::from(ctx.is_inline)));
        ...
        match result {
            ...
            Ok(val) => {
                if let Some(s) = val.as_str() {
                    ...
                    if let Ok(custom_val) = val.dollar("Error") {
                        if let Some(s) = custom_val.as_str() {
