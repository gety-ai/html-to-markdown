...
conversion_options <- function(
  ...
  opts <- list()
  if (!is.null(heading_style)) {
    opts$heading_style <- heading_style
  }
  if (!is.null(list_indent_type)) {
    opts$list_indent_type <- list_indent_type
  }
  if (!is.null(list_indent_width)) {
    opts$list_indent_width <- as.integer(list_indent_width)
  }
  if (!is.null(bullets)) {
    opts$bullets <- bullets
  }
  if (!is.null(strong_em_symbol)) {
    opts$strong_em_symbol <- strong_em_symbol
  }
  if (!is.null(escape_asterisks)) {
    opts$escape_asterisks <- escape_asterisks
  }
  if (!is.null(escape_underscores)) {
    opts$escape_underscores <- escape_underscores
  }
  if (!is.null(escape_misc)) {
    opts$escape_misc <- escape_misc
  }
  if (!is.null(escape_ascii)) {
    opts$escape_ascii <- escape_ascii
  }
  if (!is.null(code_language)) {
    opts$code_language <- code_language
  }
  if (!is.null(autolinks)) {
    opts$autolinks <- autolinks
  }
  if (!is.null(default_title)) {
    opts$default_title <- default_title
  }
  if (!is.null(br_in_tables)) {
    opts$br_in_tables <- br_in_tables
  }
  if (!is.null(compact_tables)) {
    opts$compact_tables <- compact_tables
  }
  if (!is.null(highlight_style)) {
    opts$highlight_style <- highlight_style
  }
  if (!is.null(extract_metadata)) {
    opts$extract_metadata <- extract_metadata
  }
  if (!is.null(whitespace_mode)) {
    opts$whitespace_mode <- whitespace_mode
  }
  if (!is.null(strip_newlines)) {
  ...
  if (!is.null(wrap_width)) {
    opts$wrap_width <- as.integer(wrap_width)
  }
  if (!is.null(convert_as_inline)) {
    opts$convert_as_inline <- convert_as_inline
  }
  if (!is.null(sub_symbol)) {
    opts$sub_symbol <- sub_symbol
  }
  if (!is.null(sup_symbol)) {
    opts$sup_symbol <- sup_symbol
  }
  if (!is.null(newline_style)) {
    opts$newline_style <- newline_style
  }
  if (!is.null(code_block_style)) {
    opts$code_block_style <- code_block_style
  }
  if (!is.null(keep_inline_images_in)) {
  ...
  if (!is.null(strip_tags)) {
    opts$strip_tags <- strip_tags
  }
  if (!is.null(preserve_tags)) {
    opts$preserve_tags <- preserve_tags
  }
  if (!is.null(skip_images)) {
    opts$skip_images <- skip_images
  }
  if (!is.null(url_escape_style)) {
    opts$url_escape_style <- url_escape_style
  }
  if (!is.null(link_style)) {
  ...
  if (!is.null(include_document_structure)) {
    opts$include_document_structure <- include_document_structure
  }
  if (!is.null(extract_images)) {
    opts$extract_images <- extract_images
  }
  if (!is.null(max_image_size)) {
    opts$max_image_size <- as.integer(max_image_size)
  }
  if (!is.null(capture_svg)) {
    opts$capture_svg <- capture_svg
  }
  if (!is.null(infer_dimensions)) {
    opts$infer_dimensions <- infer_dimensions
  }
  if (!is.null(max_depth)) {
    opts$max_depth <- as.integer(max_depth)
  }
  if (!is.null(exclude_selectors)) {
    opts$exclude_selectors <- exclude_selectors
  }
  if (!is.null(tier_strategy)) {
    opts$tier_strategy <- tier_strategy
  }
  if (!is.null(visitor)) {
