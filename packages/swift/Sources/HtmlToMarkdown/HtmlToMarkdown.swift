...
public struct HeaderMetadata: Codable, Sendable, Hashable {
  ...
  public func isValid() throws  -> Bool {
    ...
    let resultJson = try RustBridge.headerMetadataIsValidFromJson(selfString).toString()
...
public struct NodeContext: Codable, Sendable, Hashable {
  ...
  public func attributes() throws  -> [String: String] {
    let jsonSelf = try JSONEncoder().encode(self)
    let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
    let resultJson = try RustBridge.nodeContextAttributesFromJson(selfString).toString()
    let data = resultJson.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode([String: String].self, from: data)
  }

  public func intoOwned() throws  -> NodeContext {
    let jsonSelf = try JSONEncoder().encode(self)
    let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
    let resultJson = try RustBridge.nodeContextIntoOwnedFromJson(selfString).toString()
    let data = resultJson.data(using: .utf8) ?? Data()
