...
# alef:hash:9da11af952facc2720b10eddfafb73be64d0564d1d08696ca7cf2d4ed7adc839
...
{:ok, _} = HtmlToMarkdown.Native.convert_with_visitor(html, if(map_size(clean_opts) == 0, do: nil, else: Jason.encode!(clean_opts)))
  ...
  defp do_visitor_receive_loop(visitor) do
    receive do      {:visitor_callback, ref_id, callback_name, args_json} ->        result =
        ...
        do_visitor_receive_loop(visitor)      {:ok, result} ->
        {:ok, result}

      {:error, reason} ->
        {:error, reason}    after
      30_000 ->{:error, "visitor callback timeout after 30s"}    end
  end
  ...
  defp apply_visitor_callback(fun, args_json) do
    ...
    case result do
      :continue -> "continue"
      :skip -> "skip"
      :preserve_html -> "preserve_html"      {:custom, value} -> to_string(value)      binary when is_binary(binary) -> binary
      _ -> "continue"
    end
  end
