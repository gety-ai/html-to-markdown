...
// alef:hash:1ab2ff5f4bb9835f7577c054de2cdc476b98454451e019c37e22ce9398e024e4
...
pub fn convert(
...
) -> Result<rustler::Term<'_>, String> {
    let result = html_to_markdown_rs::convert(
        &html,
        options
    ...
    encode_conversion_result(env, &result)
}
        ...
        let _ = visitor_owned_env.send_and_clear(&pid, |env| {
            ...
            match conversion_result {
                Ok(result) => {
                    ...
                    let result_term = encode_conversion_result(env, &result).unwrap_or_else(|_| rustler::types::atom::Atom::from_str(env, "error").unwrap().to_term(env));
...
impl From<html_to_markdown_rs::metadata::DocumentMetadata> for DocumentMetadata {
    fn from(val: html_to_markdown_rs::metadata::DocumentMetadata) -> Self {
        Self {
            ...
            language: val.language,
            text_direction: val.text_direction.map(Into::into),
            open_graph: serde_json::to_string(&val.open_graph).unwrap_or_default(),
            twitter_card: serde_json::to_string(&val.twitter_card).unwrap_or_default(),
            meta_tags: serde_json::to_string(&val.meta_tags).unwrap_or_default(),
        }
    }
}
...
fn encode_conversion_result<'a>(env: rustler::Env<'a>, result: &ConversionResult) -> Result<rustler::Term<'a>, String> {
    let mut pairs: Vec<(rustler::Term, rustler::Term)> = Vec::new();

    pairs.push((
        rustler::types::atom::Atom::from_str(env, "content").unwrap().to_term(env),
        result.content.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "document").unwrap().to_term(env),
        result.document.encode(env),
    ));

    let metadata_term = encode_html_metadata(env, &result.metadata);
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "metadata").unwrap().to_term(env),
        metadata_term,
    ));

    pairs.push((
        rustler::types::atom::Atom::from_str(env, "tables").unwrap().to_term(env),
        result.tables.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "images").unwrap().to_term(env),
        result.images.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "warnings").unwrap().to_term(env),
        result.warnings.encode(env),
    ));

    Ok(rustler::Term::map_from_pairs(env, &pairs)
        .unwrap_or_else(|_| rustler::types::atom::Atom::from_str(env, "nil").unwrap().to_term(env)))
}

fn encode_html_metadata<'a>(env: rustler::Env<'a>, metadata: &HtmlMetadata) -> rustler::Term<'a> {
    let mut pairs: Vec<(rustler::Term, rustler::Term)> = Vec::new();

    let doc_term = encode_document_metadata(env, &metadata.document);
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "document").unwrap().to_term(env),
        doc_term,
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "headers").unwrap().to_term(env),
        metadata.headers.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "links").unwrap().to_term(env),
        metadata.links.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "images").unwrap().to_term(env),
        metadata.images.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "structured_data").unwrap().to_term(env),
        metadata.structured_data.encode(env),
    ));
...
fn encode_document_metadata<'a>(env: rustler::Env<'a>, meta: &DocumentMetadata) -> rustler::Term<'a> {
    let mut pairs: Vec<(rustler::Term, rustler::Term)> = Vec::new();

    pairs.push((
        rustler::types::atom::Atom::from_str(env, "title").unwrap().to_term(env),
        meta.title.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "description").unwrap().to_term(env),
        meta.description.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "keywords").unwrap().to_term(env),
        meta.keywords.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "author").unwrap().to_term(env),
        meta.author.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "canonical_url").unwrap().to_term(env),
        meta.canonical_url.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "base_href").unwrap().to_term(env),
        meta.base_href.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "language").unwrap().to_term(env),
        meta.language.encode(env),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "text_direction").unwrap().to_term(env),
        meta.text_direction.encode(env),
    ));

    pairs.push((
        rustler::types::atom::Atom::from_str(env, "open_graph").unwrap().to_term(env),
        json_string_to_map(env, &meta.open_graph),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "twitter_card").unwrap().to_term(env),
        json_string_to_map(env, &meta.twitter_card),
    ));
    pairs.push((
        rustler::types::atom::Atom::from_str(env, "meta_tags").unwrap().to_term(env),
        json_string_to_map(env, &meta.meta_tags),
    ));
...
fn json_string_to_map<'a>(env: rustler::Env<'a>, json_str: &str) -> rustler::Term<'a> {
    if json_str.is_empty() {
        let empty: Vec<(rustler::Term, rustler::Term)> = vec![];
        return rustler::Term::map_from_pairs(env, &empty)
            .unwrap_or_else(|_| rustler::types::atom::Atom::from_str(env, "nil").unwrap().to_term(env));
    }

    if let Ok(serde_json::Value::Object(map)) = serde_json::from_str::<serde_json::Value>(json_str) {
        let pairs: Vec<(rustler::Term, rustler::Term)> = map
            .iter()
            .map(|(k, v)| {
                let key_term = k.encode(env);
                let value_str = match v {
                    serde_json::Value::String(s) => s.clone(),
                    _ => v.to_string(),
                };
                (key_term, value_str.encode(env))
            })
            .collect();
        rustler::Term::map_from_pairs(env, &pairs)
            .unwrap_or_else(|_| {
                let empty: Vec<(rustler::Term, rustler::Term)> = vec![];
                rustler::Term::map_from_pairs(env, &empty).unwrap()
            })
    } else {
        let empty: Vec<(rustler::Term, rustler::Term)> = vec![];
        rustler::Term::map_from_pairs(env, &empty)
            .unwrap_or_else(|_| {
                let empty2: Vec<(rustler::Term, rustler::Term)> = vec![];
                rustler::Term::map_from_pairs(env, &empty2).unwrap()
            })
    }
}

rustler::init!("Elixir.HtmlToMarkdown.Native");
