...
// alef:hash:54802c14996cc39f52d1919c645d78e14d2ead10757ee774619eb845bd6cdad9
...
public static class HtmlToMarkdownRs
{
    ...
    public static ConversionResult Convert(string html, ConversionOptions? options)
    {
        ...
        // options-field bridge: build options handle via update pattern
        var optionsHandle = IntPtr.Zero;
        if (options != null)
        {
            var optionsJson = JsonSerializer.Serialize(options, JsonOptions);
            var optionsUpdateHandle = NativeMethods.ConversionOptionsUpdateFromJson(optionsJson);
            optionsHandle = NativeMethods.ConversionOptionsFromUpdate(optionsUpdateHandle);
            NativeMethods.ConversionOptionsUpdateFree(optionsUpdateHandle);
        }
        else
        {
            optionsHandle = NativeMethods.ConversionOptionsDefault();
        }
        // options-field bridge: attach visitor when present
        if (options != null && options.Visitor != null)
        {
            NativeMethods.ConversionOptionsSetVisitor(optionsHandle, options.Visitor._vtable);
        }
