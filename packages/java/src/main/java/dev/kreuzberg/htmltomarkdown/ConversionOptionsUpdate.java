// This file is auto-generated by alef — DO NOT EDIT.
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef
package dev.kreuzberg.htmltomarkdown;

import java.util.List;
import java.util.Optional;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonInclude;
import org.jetbrains.annotations.Nullable;

/**
 * Partial update for {@code ConversionOptions}.
 *
 * Uses {@code Option&lt;T&gt;} fields for selective updates. Bindings use this to construct
 * options from language-native types. Prefer ConversionOptionsBuilder for Rust code.
 */
@JsonInclude(JsonInclude.Include.NON_ABSENT)
public record ConversionOptionsUpdate(
    /** Optional override for [{@code ConversionOptions::heading_style}]. */
    @Nullable @JsonProperty("heading_style") HeadingStyle headingStyle,
    /** Optional override for [{@code ConversionOptions::list_indent_type}]. */
    @Nullable @JsonProperty("list_indent_type") ListIndentType listIndentType,
    /** Optional override for [{@code ConversionOptions::list_indent_width}]. */
    @Nullable @JsonProperty("list_indent_width") Long listIndentWidth,
    /** Optional override for [{@code ConversionOptions::bullets}]. */
    @Nullable String bullets,
    /** Optional override for [{@code ConversionOptions::strong_em_symbol}]. */
    @Nullable @JsonProperty("strong_em_symbol") String strongEmSymbol,
    /** Optional override for [{@code ConversionOptions::escape_asterisks}]. */
    @Nullable @JsonProperty("escape_asterisks") Boolean escapeAsterisks,
    /** Optional override for [{@code ConversionOptions::escape_underscores}]. */
    @Nullable @JsonProperty("escape_underscores") Boolean escapeUnderscores,
    /** Optional override for [{@code ConversionOptions::escape_misc}]. */
    @Nullable @JsonProperty("escape_misc") Boolean escapeMisc,
    /** Optional override for [{@code ConversionOptions::escape_ascii}]. */
    @Nullable @JsonProperty("escape_ascii") Boolean escapeAscii,
    /** Optional override for [{@code ConversionOptions::code_language}]. */
    @Nullable @JsonProperty("code_language") String codeLanguage,
    /** Optional override for [{@code ConversionOptions::autolinks}]. */
    @Nullable Boolean autolinks,
    /** Optional override for [{@code ConversionOptions::default_title}]. */
    @Nullable @JsonProperty("default_title") Boolean defaultTitle,
    /** Optional override for [{@code ConversionOptions::br_in_tables}]. */
    @Nullable @JsonProperty("br_in_tables") Boolean brInTables,
    /** Optional override for [{@code ConversionOptions::highlight_style}]. */
    @Nullable @JsonProperty("highlight_style") HighlightStyle highlightStyle,
    /** Optional override for [{@code ConversionOptions::extract_metadata}]. */
    @Nullable @JsonProperty("extract_metadata") Boolean extractMetadata,
    /** Optional override for [{@code ConversionOptions::whitespace_mode}]. */
    @Nullable @JsonProperty("whitespace_mode") WhitespaceMode whitespaceMode,
    /** Optional override for [{@code ConversionOptions::strip_newlines}]. */
    @Nullable @JsonProperty("strip_newlines") Boolean stripNewlines,
    /** Optional override for [{@code ConversionOptions::wrap}]. */
    @Nullable Boolean wrap,
    /** Optional override for [{@code ConversionOptions::wrap_width}]. */
    @Nullable @JsonProperty("wrap_width") Long wrapWidth,
    /** Optional override for [{@code ConversionOptions::convert_as_inline}]. */
    @Nullable @JsonProperty("convert_as_inline") Boolean convertAsInline,
    /** Optional override for [{@code ConversionOptions::sub_symbol}]. */
    @Nullable @JsonProperty("sub_symbol") String subSymbol,
    /** Optional override for [{@code ConversionOptions::sup_symbol}]. */
    @Nullable @JsonProperty("sup_symbol") String supSymbol,
    /** Optional override for [{@code ConversionOptions::newline_style}]. */
    @Nullable @JsonProperty("newline_style") NewlineStyle newlineStyle,
    /** Optional override for [{@code ConversionOptions::code_block_style}]. */
    @Nullable @JsonProperty("code_block_style") CodeBlockStyle codeBlockStyle,
    /** Optional override for [{@code ConversionOptions::keep_inline_images_in}]. */
    @Nullable @JsonProperty("keep_inline_images_in") List<String> keepInlineImagesIn,
    /** Optional override for [{@code ConversionOptions::preprocessing}]. */
    @Nullable PreprocessingOptionsUpdate preprocessing,
    /** Optional override for [{@code ConversionOptions::encoding}]. */
    @Nullable String encoding,
    /** Optional override for [{@code ConversionOptions::debug}]. */
    @Nullable Boolean debug,
    /** Optional override for [{@code ConversionOptions::strip_tags}]. */
    @Nullable @JsonProperty("strip_tags") List<String> stripTags,
    /** Optional override for [{@code ConversionOptions::preserve_tags}]. */
    @Nullable @JsonProperty("preserve_tags") List<String> preserveTags,
    /** Optional override for [{@code ConversionOptions::skip_images}]. */
    @Nullable @JsonProperty("skip_images") Boolean skipImages,
    /** Optional override for [{@code ConversionOptions::link_style}]. */
    @Nullable @JsonProperty("link_style") LinkStyle linkStyle,
    /** Optional override for [{@code ConversionOptions::output_format}]. */
    @Nullable @JsonProperty("output_format") OutputFormat outputFormat,
    /** Optional override for [{@code ConversionOptions::include_document_structure}]. */
    @Nullable @JsonProperty("include_document_structure") Boolean includeDocumentStructure,
    /** Optional override for [{@code ConversionOptions::extract_images}]. */
    @Nullable @JsonProperty("extract_images") Boolean extractImages,
    /** Optional override for [{@code ConversionOptions::max_image_size}]. */
    @Nullable @JsonProperty("max_image_size") Long maxImageSize,
    /** Optional override for [{@code ConversionOptions::capture_svg}]. */
    @Nullable @JsonProperty("capture_svg") Boolean captureSvg,
    /** Optional override for [{@code ConversionOptions::infer_dimensions}]. */
    @Nullable @JsonProperty("infer_dimensions") Boolean inferDimensions,
    /** Optional override for [{@code ConversionOptions::max_depth}]. */
    @Nullable @JsonProperty("max_depth") Long maxDepth,
    /** Optional override for [{@code ConversionOptions::exclude_selectors}]. */
    @Nullable @JsonProperty("exclude_selectors") List<String> excludeSelectors,
    /** Optional override for [{@code ConversionOptions::visitor}]. */
    @Nullable VisitorHandle visitor
) {
    public static ConversionOptionsUpdateBuilder builder() {
        return new ConversionOptionsUpdateBuilder();
    }
    /** Returns an Optional wrapping the bullets value. */
    public java.util.Optional<String> bullets() {
        return java.util.Optional.ofNullable(this.bullets);
    }
    /** Returns an Optional wrapping the codeLanguage value. */
    public java.util.Optional<String> codeLanguage() {
        return java.util.Optional.ofNullable(this.codeLanguage);
    }
    /** Returns an Optional wrapping the subSymbol value. */
    public java.util.Optional<String> subSymbol() {
        return java.util.Optional.ofNullable(this.subSymbol);
    }
    /** Returns an Optional wrapping the supSymbol value. */
    public java.util.Optional<String> supSymbol() {
        return java.util.Optional.ofNullable(this.supSymbol);
    }
    /** Returns an Optional wrapping the encoding value. */
    public java.util.Optional<String> encoding() {
        return java.util.Optional.ofNullable(this.encoding);
    }
}
