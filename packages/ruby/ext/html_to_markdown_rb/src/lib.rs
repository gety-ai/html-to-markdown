...
// alef:hash:286cd6451bbaf9ebafa13e54058f2c5fe41745c9752725e0396b34f7a97eeddb
...
fn convert(args: &[magnus::Value]) -> Result<ConversionResult, Error> {
    let args = magnus::scan_args::scan_args::<((String,), (Option<magnus::Value>,))>(args)?;
    let (html) = args.required;
    let (options) = args.optional;
    let options: Option<ConversionOptions> = match options {
        Some(_v) if !_v.is_nil() => {
            let s: String = _v.funcall("to_json", ())?;
