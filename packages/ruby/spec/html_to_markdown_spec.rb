...
RSpec.describe HtmlToMarkdown do
  describe ".convert" do
    ...
    it "applies heading_style option" do
      ...
      expect(result.to_s).to(match(/^# Hello/))
    end
    ...
    it "returns a ConversionResult with string representation" do
      expect(described_class.convert("<p>test</p>").to_s).to(be_a(String))
      expect(described_class.convert("<p>test</p>", strip_newlines: true).to_s).to(be_a(String))
    end
  end
end
