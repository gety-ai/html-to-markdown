...
test_that("form_input_elements: Form input elements produce readable output without form mechanics", {
  result <- convert(html = "<form><label for=\"name\">Name:</label><input type=\"text\" id=\"name\" placeholder=\"Enter name\"></form>", options = list("preprocessing" = list("remove_forms" = FALSE)))
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("form_select_options: Select element with options produces readable output", {
  result <- convert(html = "<form><label>Color:</label><select><option value=\"red\">Red</option><option value=\"blue\" selected>Blue</option><option value=\"green\">Green</option></select></form>", options = list("preprocessing" = list("remove_forms" = FALSE)))
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
  expect_true(grepl("Color", result$content, fixed = TRUE))
})

test_that("form_textarea: Textarea element produces readable output", {
  result <- convert(html = "<form><label>Message:</label><textarea>Default text content</textarea></form>", options = list("preprocessing" = list("remove_forms" = FALSE)))
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
