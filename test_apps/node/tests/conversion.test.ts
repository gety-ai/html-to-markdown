...
describe("conversion", () => {
	...
	it("form_input_elements: Form input elements produce readable output without form mechanics", () => {
		const result = convert("<form><label for=\"name\">Name:</label><input type=\"text\" id=\"name\" placeholder=\"Enter name\"></form>", { preprocessing: { removeForms: false } } as ConversionOptions);
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
	...
	it("form_select_options: Select element with options produces readable output", () => {
		const result = convert("<form><label>Color:</label><select><option value=\"red\">Red</option><option value=\"blue\" selected>Blue</option><option value=\"green\">Green</option></select></form>", { preprocessing: { removeForms: false } } as ConversionOptions);
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
	...
	it("form_textarea: Textarea element produces readable output", () => {
		const result = convert("<form><label>Message:</label><textarea>Default text content</textarea></form>", { preprocessing: { removeForms: false } } as ConversionOptions);
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
