...
## Swift API Reference <span class="version-badge">v3.6.10</span>
...
| `listIndentWidth` | `UInt64` | `2` | Number of spaces (or tabs) to use for each level of list indentation. |
...
| `wrapWidth` | `UInt64` | `80` | Maximum output line width in characters when `wrap` is `true` (default `80`). Lines are broken at word boundaries so that no line exceeds this length. A value of `0` is treated as "no limit" — equivalent to leaving `wrap` disabled. Has no effect when `wrap` is `false`. |
...
| `maxDepth` | `UInt64?` | `null` | Maximum DOM traversal depth. `null` means unlimited. When set, subtrees beyond this depth are silently truncated. |
...
| `tables` | `[TableData]` | `[]` | Extracted tables with structured cell data and markdown representation. |
| `images` | `[String]` | `[]` | Extracted inline images from data URIs and SVGs. Populated when the `inline-images` feature is enabled and `extract_images` is `true`. Bindings may expose a simplified image representation or omit this Rust-only payload depending on backend support for binary image data. |
| `warnings` | `[ProcessingWarning]` | `[]` | Non-fatal processing warnings. |
...
| `parent` | `UInt32?` | `null` | Index of the parent node (None for root nodes). |
...
| `images` | `[ImageMetadata]` | `[]` | Extracted images with source and dimensions |
...
| `attributes` | `[String: String]` | — | Additional HTML attributes |
...
| `depth` | `UInt64` | — | Depth in the DOM tree (0 = root) |
