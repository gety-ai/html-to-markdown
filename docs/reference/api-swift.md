...
| `wrapWidth` | `UInt64` | `80` | Maximum line width when `wrap` is enabled (default `80`). |
...
| `attributes` | `[String: String]?` | `null` | Format-specific attributes (e.g. class, id, data-* attributes). |
...
| `cells` | `[GridCell]` | `[]` | All cells in the table (may be fewer than rows*cols due to spans). |
