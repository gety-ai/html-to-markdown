...
## Zig API Reference <span class="version-badge">v3.6.10</span>
...
| `images` | `[]const [:0]const u8` | `[]` | Extracted inline images from data URIs and SVGs. Populated when the `inline-images` feature is enabled and `extract_images` is `true`. Bindings may expose a simplified image representation or omit this Rust-only payload depending on backend support for binary image data. |
