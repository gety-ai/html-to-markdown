...
## R API Reference <span class="version-badge">v3.8.1</span>
...
| `max_depth` | `integer or NULL` | `NULL` | Maximum DOM traversal depth. `NULL` uses the library's internal native-stack safety limit. Explicit values above that safety limit are clamped to prevent process-aborting stack overflows on pathologically deep DOM trees. |
