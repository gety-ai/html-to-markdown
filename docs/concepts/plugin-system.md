---
description: "html-to-markdown plugin system — the HtmlVisitor trait, 40 callbacks, VisitResult variants, and binding support."
---
...
Rust users opt in with `features = ["visitor"]`. Bindings expose the same hooks only where the generated backend supports trait handles; Kotlin Android excludes visitor support.
...
pub trait HtmlVisitor: std::fmt::Debug + Send {
    ...
    // ... 34 more element-specific methods, all with `Continue` defaults
}
```

40 methods in total: text and element pre/post hooks plus a method per HTML element family (links, images, headings, lists, code, tables, definition lists, and forms/media/figures). Override only the methods you need.
...
let visitor: VisitorHandle = Arc::new(Mutex::new(MyVisitor::default()));
...
When a visitor crosses the C FFI boundary, the C layer exposes a `HTMHtmVisitorCallbacks` struct of function pointers. The Rust side wraps each callback in a bridge that marshals strings and `NodeContext` fields and translates the returned status code into a `VisitResult`. The status codes are `HTM_VISIT_CONTINUE`, `HTM_VISIT_SKIP`, `HTM_VISIT_PRESERVE_HTML`, `HTM_VISIT_CUSTOM`, and `HTM_VISIT_ERROR`.
