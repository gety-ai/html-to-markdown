...
// alef:hash:d1a2c661d4b7da80d3355b9e17544225c0e984291a61ca2eff21e33097c5510c
...
fn php_zval_to_visit_result(val: &ext_php_rs::types::Zval) -> html_to_markdown_rs::VisitResult {
    // Try string first (bare strings like "continue", "skip", "preserve_html")
    if val.is_string() {
        if let Some(s) = val.string() {
            let s_lower = s.to_lowercase();
            return match s_lower.as_str() {
    ...
    // Try array (e.g., ["custom" => "value"] or ["error" => "message"])
    if val.is_array() {
        if let Some(arr) = val.array() {
            for (key, value) in arr.iter() {
                let key_str = key.to_string().to_lowercase();
                if key_str == "custom" {
                    if let Some(custom_val) = value.string() {
                        return html_to_markdown_rs::VisitResult::Custom(custom_val.to_string());
                    }
                }
                if key_str == "error" {
                    if let Some(error_val) = value.string() {
                        return html_to_markdown_rs::VisitResult::Error(error_val.to_string());
                    }
                }
            }
        }
...
impl From<ConversionOptions> for html_to_markdown_rs::options::ConversionOptions {
    fn from(val: ConversionOptions) -> Self {
        Self {
            ...
            visitor: val.visitor.as_ref().map(|v| (*v.inner).clone()),
        }
    }
}
