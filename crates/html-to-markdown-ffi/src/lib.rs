...
// alef:hash:3f969bbc05d7f899ee2d50a5488136b0aa64eaad872fd4c0a35a7a9c7052109f
...
pub unsafe extern "C" fn htm_conversion_options_builder_strip_tags(
    ...
    let tags_rs = match serde_json::from_str::<Vec<String>>(tags_rs_str) {
        Ok(v) => v,
        Err(e) => {
            set_last_error(2, &e.to_string());
            return std::ptr::null_mut();
...
pub unsafe extern "C" fn htm_conversion_options_builder_exclude_selectors(
    ...
    let selectors_rs = match serde_json::from_str::<Vec<String>>(selectors_rs_str) {
        Ok(v) => v,
        Err(e) => {
            set_last_error(2, &e.to_string());
            return std::ptr::null_mut();
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn htm_visitor_handle_from_callbacks(
    callbacks: *const HtmVisitorCallbacks,
) -> *mut html_to_markdown_rs::visitor::VisitorHandle {
    if callbacks.is_null() {
        return std::ptr::null_mut();
    }
    // SAFETY: caller guarantees the pointer is valid for read.
    let cbs = unsafe { callbacks.read() };
    let visitor = HtmVisitor {
        callbacks: cbs,
        _tag_scratch: std::cell::RefCell::new(Vec::new()),
    };
    let handle: html_to_markdown_rs::visitor::VisitorHandle =
        std::sync::Arc::new(std::sync::Mutex::new(visitor));
...
/// subsequent `htm_convert` call.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn htm_options_set_visitor_handle(
    options: *mut html_to_markdown_rs::ConversionOptions,
    ...
    // SAFETY: VisitorRef is a thin wrapper around a raw pointer to HtmVisitor which
...
pub unsafe extern "C" fn htm_convert_with_visitor(
    ...
    let visitor_handle: Option<std::sync::Arc<std::sync::Mutex<dyn html_to_markdown_rs::visitor::HtmlVisitor + Send>>> =
        if visitor.is_null() {
            None
        } else {
    ...
    let mut options_with_visitor: Option<html_to_markdown_rs::ConversionOptions> = options_rs;
    if visitor_handle.is_some() {
        let opts = options_with_visitor.get_or_insert_with(html_to_markdown_rs::ConversionOptions::default);
        opts.visitor = visitor_handle;
    }
    match html_to_markdown_rs::convert(&html_str, options_with_visitor) {
        Ok(result) => Box::into_raw(Box::new(result)),
        Err(e) => {
            set_last_error(2, &e.to_string());
            std::ptr::null_mut()
