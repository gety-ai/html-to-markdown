...
// alef:hash:ce044e4e24f65fdb05285c272d4b0484e38cca4891a03a38790ed259b276ff3b
                ...
                if val.get_type().ok() == Some(napi::bindgen_prelude::ValueType::Object) {
                    if let Ok(obj) = val.coerce_to_object() {
                        ...
                        if let Ok(ev) = obj.get_named_property::<napi::bindgen_prelude::Unknown>("error") {
                            if !matches!(
                                ev.get_type().unwrap_or(napi::bindgen_prelude::ValueType::Undefined),
                                napi::bindgen_prelude::ValueType::Undefined | napi::bindgen_prelude::ValueType::Null
                            ) {
                                if let Ok(s) = ev
                                    .coerce_to_string()
...
impl From<html_to_markdown_rs::options::ConversionOptions> for JsConversionOptions {
    fn from(val: html_to_markdown_rs::options::ConversionOptions) -> Self {
        Self {
            heading_style: Some(format!("{:?}", val.heading_style)),
            list_indent_type: Some(format!("{:?}", val.list_indent_type)),
            ...
            highlight_style: Some(format!("{:?}", val.highlight_style)),
            extract_metadata: Some(val.extract_metadata),
            whitespace_mode: Some(format!("{:?}", val.whitespace_mode)),
            ...
            newline_style: Some(format!("{:?}", val.newline_style)),
            code_block_style: Some(format!("{:?}", val.code_block_style)),
            ...
            link_style: Some(format!("{:?}", val.link_style)),
...
impl From<JsConversionOptionsUpdate> for html_to_markdown_rs::options::ConversionOptionsUpdate {
    fn from(val: JsConversionOptionsUpdate) -> Self {
        ...
        __result.compact_tables = val.compact_tables;
        ...
        __result.keep_inline_images_in = val.keep_inline_images_in.map(|v| v.into_iter().collect());
        __result.preprocessing = val.preprocessing.map(Into::into);
        __result.encoding = val.encoding;
        __result.debug = val.debug;
        __result.strip_tags = val.strip_tags.map(|v| v.into_iter().collect());
        __result.preserve_tags = val.preserve_tags.map(|v| v.into_iter().collect());
        ...
        __result.exclude_selectors = val.exclude_selectors.map(|v| v.into_iter().collect());
        __result.visitor = Default::default();
        __result
    }
}
