...
pub fn convert(html: String, options: Option<JsConversionOptions>) -> Result<JsConversionResult> {
    let visitor_handle: Option<html_to_markdown_rs::VisitorHandle> =
        options.as_ref().and_then(|o| o.visitor.as_ref()).and_then(|v| {
            JsHtmlVisitorBridge::new(v.clone())
                .ok()
...
impl From<JsConversionOptions> for html_to_markdown_rs::options::ConversionOptions {
    fn from(val: JsConversionOptions) -> Self {
        ...
        __result.visitor = val.visitor.and_then(|obj| {
            JsHtmlVisitorBridge::new(obj)
                .ok()
                .map(|bridge| std::sync::Arc::new(std::sync::Mutex::new(bridge)) as html_to_markdown_rs::VisitorHandle)
        });
        __result
