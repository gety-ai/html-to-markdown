...
- **task(rust:test): exclude `html-to-markdown-rs-dart` from `cargo test --no-default-features --workspace`.** The dart wrapper crate's mirror enums (`VisitResult`, `ImageType`) are gated on the crate's own `visitor` / `metadata` features, but the companion `frb_generated.rs` emitted by `flutter_rust_bridge` references those mirrors via `crate::T` unconditionally — and with default features off, the gated declarations vanish and the generated file fails `E0433: cannot find 'T' in 'crate'`. CI E2E Test: Node (all three platforms) was red on v3.6.13. The companion alef fix (drop `source_cfg` from `rust_mirror_{enum,struct}_attribute.jinja` so the mirror declarations stay unconditional, matching the long-standing variant policy) is queued in alef [Unreleased]; once the next alef cut + regen lands, this exclusion can be lifted. (`.task/languages/rust.yml`)
