...
- **bindings: regenerated with alef 0.19.1 (final).** Picks up the Swift FFI fix: `packages/swift/Sources/RustBridgeC/RustBridgeC.h` now contains the full concatenated SwiftBridgeCore + binding-crate headers (was a placeholder under 0.19.0, breaking `swift build` with `cannot find type 'RustStr' in scope`); `packages/swift/Sources/HtmlToMarkdown/HtmlToMarkdown.swift` gains complete `Codable` conformance for `NodeContent` (custom `init(from:)`/`encode(to:)` over the tag/value layout); JNI gains a `jni_call_string_method` helper consolidating the boilerplate around `Object`-returning `String` method calls; magnus (Ruby) and rustler (Elixir) `NodeContent::MetadataBlock` reverse conversion now rebuilds `Vec<(String, String)>` from the sanitized `Vec<Vec<String>>` binding shape — the prior `entries.iter().filter_map(|s| serde_json::from_str(s).ok()).collect()` did not type-check (`expected '&str', found '&Vec<String>'`) and broke every macOS/Ubuntu/Windows × Ruby 3.2/3.3 build in CI. `alef sync-versions` also re-resolved `test_apps/` dependency pins from the published registries (npm/PyPI/NuGet/RubyGems/Maven/Crates/Go/CRAN) — h2m's last successful publish is `3.4.0`, so the e2e test apps now reference `3.4.0` (or the rc tag where pinned), not the unreleased `3.5.0-rc.1` workspace version.
