...
The preprocessing pass is conservative — it only removes elements that would never contribute to text output. See the [Configuration → Preprocessing](/configuration/#preprocessing) section for the exact rules.
...
- Dispatches to the registered [`HtmlVisitor`](/concepts/plugin-system/) (if any) and applies the returned `VisitResult`.
