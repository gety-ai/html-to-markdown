...
RSpec.describe "metadata" do
  ...
  it(
    "metadata_image_type_data_uri_classified: A data-URI image is extracted into metadata.images when extract_metadata is true"
  ) do
    result = HtmlToMarkdown.convert(
  ...
  it(
    "metadata_image_type_external_classified: An https image URL is extracted into metadata.images when extract_metadata is true"
  ) do
    result = HtmlToMarkdown.convert(
  ...
  it(
    "metadata_link_type_anchor_classified: A fragment anchor link is extracted into metadata.links when extract_metadata is true"
  ) do
    result = HtmlToMarkdown.convert(
      "<p>Jump to <a href=\"#section\">section</a> below.</p>",
      HtmlToMarkdownRs::ConversionOptions.new(extract_metadata: true)
    )
    expect(result.content.to_s).not_to(be_empty)
  ...
  it "metadata_link_type_email_classified: A mailto link is extracted into metadata.links when extract_metadata is true" do
    result = HtmlToMarkdown.convert(
      "<p>Contact <a href=\"mailto:hello@example.com\">us</a> directly.</p>",
      HtmlToMarkdownRs::ConversionOptions.new(extract_metadata: true)
    )
    expect(result.content.to_s).not_to(be_empty)
    expect(result.metadata.links.length).to(be >= 1)
  ...
  it(
    "metadata_link_type_external_classified: An https link is extracted into metadata.links when extract_metadata is true"
  ) do
    result = HtmlToMarkdown.convert(
  ...
  it "metadata_microdata_schema_breadcrumb: Extract schema.org breadcrumb navigation microdata" do
    result = HtmlToMarkdown.convert(
      ...
      HtmlToMarkdownRs::ConversionOptions.new(extract_metadata: true, preprocessing: {"remove_navigation" => false})
    )
    expect(result.content.to_s).not_to(be_empty)
