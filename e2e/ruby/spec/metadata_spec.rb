...
# alef:hash:e163d6056100ce945d0a5b99c7bfd2fb208e3aec549f930543be19a1dfb822dd
...
RSpec.describe 'metadata' do
  it 'metadata_author_meta: Extract author from <meta name=\'author\'> tag' do
    ...
    expect(result.metadata.document.author).to eq('Jane Doe')
  ...
  it 'metadata_canonical_url: Extract canonical URL from <link rel=\'canonical\'> tag' do
    ...
    expect(result.content.to_s).not_to be_empty
    expect(result.metadata.document.canonical_url).to eq('https://example.com/canonical-page')
  ...
  it 'metadata_description_meta: Extract description from <meta name=\'description\'> tag' do
    ...
    expect(result.content.to_s).not_to be_empty
    expect(result.metadata.document.description).to eq('This is the page description.')
  ...
  it 'metadata_title_tag: Extract title from <title> tag' do
    ...
    expect(result.content.to_s).not_to be_empty
    expect(result.metadata.document.title).to eq('My Page')
  ...
  it 'og_basic_tags: Extract og:title, og:description, and og:image from Open Graph meta tags' do
    ...
    expect(result.content.to_s).not_to be_empty
    expect(result.metadata.document.open_graph.get("title")).to eq('OG Title')
    expect(result.metadata.document.open_graph.get("description")).to eq('OG description text.')
    expect(result.metadata.document.open_graph.get("image")).to eq('https://example.com/image.jpg')
  ...
  it 'og_multiple_tags: Extract multiple Open Graph tags including type, url, and site_name' do
    ...
    expect(result.content.to_s).not_to be_empty
    expect(result.metadata.document.open_graph.get("title")).to eq('Article Title')
    expect(result.metadata.document.open_graph.get("type")).to eq('article')
    expect(result.metadata.document.open_graph.get("url")).to eq('https://example.com/article')
    expect(result.metadata.document.open_graph.get("site_name")).to eq('Example Site')
  ...
  it 'twitter_card_tags: Extract Twitter card meta tags' do
    ...
    expect(result.content.to_s).not_to be_empty
    expect(result.metadata.document.twitter_card.get("card")).to eq('summary_large_image')
    expect(result.metadata.document.twitter_card.get("title")).to eq('Twitter Card Title')
    expect(result.metadata.document.twitter_card.get("description")).to eq('Twitter card description.')
