...
RSpec.describe "real-world" do
  it "real_world_blog_post: Blog post with headings, paragraphs, code blocks, and links converts to readable Markdown" do
    ...
    expect(result.content.to_s).to(include("# Getting Started with Rust"))
    expect(result.content.to_s).to(include("## Installation"))
    expect(result.content.to_s).to(include("## Hello World"))
    expect(result.content.to_s).to(include("## Key Concepts"))
    expect(result.content.to_s).to(include("cargo run"))
    expect(result.content.to_s).to(include("[Mozilla](https://www.mozilla.org)"))
    expect(result.content.to_s).to(include("- Ownership and borrowing"))
  end

  it(
    "real_world_documentation_page: Documentation page with nested lists, code examples, and blockquotes converts correctly"
  ) do
    result = HtmlToMarkdown.convert(
    ...
    expect(result.content.to_s).not_to(be_empty)
    expect(result.content.to_s).to(include("# API Reference"))
    expect(result.content.to_s).to(include("## convert_html"))
    expect(result.content.to_s).to(include("### Parameters"))
    expect(result.content.to_s).to(include("### Returns"))
    expect(result.content.to_s).to(include("### Example"))
    expect(result.content.to_s).to(include("## ConversionOptions"))
    expect(result.content.to_s).to(include("> "))
    expect(result.content.to_s).to(include("thread-safe"))
    expect(result.content.to_s).to(include("convert_html"))
    expect(result.content.to_s).to(include("ConversionOptions"))
  end
  ...
  it "real_world_product_page: Product page with table, images, and lists converts correctly" do
    ...
    expect(result.content.to_s).not_to(be_empty)
    expect(result.content.to_s).to(include("# Wireless Keyboard Pro"))
    expect(result.content.to_s).to(include("![Wireless Keyboard Pro](https://example.com/keyboard.jpg)"))
    expect(result.content.to_s).to(include("## Specifications"))
    expect(result.content.to_s).to(include("Battery Life"))
    expect(result.content.to_s).to(include("12 months"))
    expect(result.content.to_s).to(include("Bluetooth 5.0"))
    expect(result.content.to_s).to(include("\#\# What's in the Box"))
    expect(result.content.to_s).to(include("USB-C charging cable"))
    expect(result.content.to_s).to(include("|"))
    expect(result.content.to_s).to(include("---"))
  end
end
