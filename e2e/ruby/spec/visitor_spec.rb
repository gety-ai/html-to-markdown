...
RSpec.describe "visitor" do
  ...
  it(
    "visitor_heading_bare_string_preserves_case: Visitor returns rendered heading template as bare string; mixed-case is preserved (regression guard for issue #350)"
  ) do
    visitor = Class
      .new do
        def visit_heading(ctx, level, text, id)
  ...
  it(
    "visitor_image_bare_string_preserves_case: Visitor returns bare-string image replacement; mixed-case alt/src preserved (regression guard for issue #350)"
  ) do
    visitor = Class
      .new do
        def visit_image(ctx, src, alt, title)
  ...
  it(
    "visitor_link_bare_string_preserves_case: Visitor returns rendered template as a bare string (not a dict) and original mixed-case content is preserved end-to-end (regression guard for issue #350)"
  ) do
    visitor = Class
      .new do
        def visit_link(ctx, href, text, title)
