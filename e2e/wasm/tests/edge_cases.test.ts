...
// alef:hash:34507639122e46a931bfab60be5045f1e70bcecc8e5e4ef6509e20f381bfca63
...
describe("edge-cases", () => {
  it("empty_html: Empty HTML document", () => {
    const result = convert("<html><head></head><body></body></html>", {} as WasmConversionOptions);
  ...
  it("malformed_overlapping_tags: Overlapping bold/italic tags are recovered by the HTML parser without panic", () => {
    const result = convert("<p><b><i>bold and italic</b></i></p>", {} as WasmConversionOptions);
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it("malformed_unclosed_paragraph: Unclosed <p> tag is recovered gracefully and content is preserved", () => {
    const result = convert("<p>This paragraph is never closed", {} as WasmConversionOptions);
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it("visitor_element_start_skip_entire_subtree: Visitor skips at element_start level removes entire subtree", () => {
    ...
    const result = convert("<div><h1>Title</h1><p>Content</p></div>", {} as WasmConversionOptions, {
      visitor: _testVisitor,
    });
  ...
  it("whitespace_only: Whitespace-only content", () => {
    const result = convert("<p>   </p>", {} as WasmConversionOptions);
    expect((result.content ?? "").trim()).toBe("");
  });
