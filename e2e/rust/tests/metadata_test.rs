...
// alef:hash:df4fa70c91f1fbc35cad452443bea4be8351f975e6e046e5be3803b274ede0e8
...
fn test_metadata_author_meta() {
    ...
    let metadata_document_author = result
        .metadata
        .document
        .author
        .as_ref()
    ...
    assert_eq!(
        metadata_document_author.to_string().as_str().trim(),
        r#"Jane Doe"#,
...
fn test_metadata_canonical_url() {
    ...
    let options: ConversionOptions = serde_json::from_value(options_json).unwrap();
    let result = convert(html, Some(options.clone())).expect("should succeed");
    let metadata_document_canonical_url = result
        .metadata
        .document
        .canonical_url
        .as_ref()
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        metadata_document_canonical_url.to_string().as_str().trim(),
...
fn test_metadata_microdata_schema_breadcrumb() {
    ...
    let options_json = serde_json::json!({"extract_metadata": true, "preprocessing": {"remove_navigation": false}});
    let options: ConversionOptions = serde_json::from_value(options_json).unwrap();
    let result = convert(html, Some(options.clone())).expect("should succeed");
    let content = result.content.as_ref().map(|v| v.to_string()).unwrap_or_default();
    assert!(!content.is_empty(), "expected non-empty value");
...
fn test_og_basic_tags() {
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        metadata_document_open_graph_title.to_string().as_str().trim(),
        r#"OG Title"#,
        "equals assertion failed"
    );
    assert_eq!(
        metadata_document_open_graph_description.to_string().as_str().trim(),
        r#"OG description text."#,
        "equals assertion failed"
    );
    assert_eq!(
        metadata_document_open_graph_image.to_string().as_str().trim(),
...
fn test_og_multiple_tags() {
    ...
    assert_eq!(
        metadata_document_open_graph_type.to_string().as_str().trim(),
        r#"article"#,
        "equals assertion failed"
    );
    assert_eq!(
        metadata_document_open_graph_url.to_string().as_str().trim(),
        r#"https://example.com/article"#,
        "equals assertion failed"
    );
    assert_eq!(
        metadata_document_open_graph_site_name.to_string().as_str().trim(),
        r#"Example Site"#,
        "equals assertion failed"
    );
}
...
fn test_twitter_card_tags() {
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        metadata_document_twitter_card_card.to_string().as_str().trim(),
        r#"summary_large_image"#,
        "equals assertion failed"
    );
    assert_eq!(
        metadata_document_twitter_card_title.to_string().as_str().trim(),
        r#"Twitter Card Title"#,
        "equals assertion failed"
    );
    assert_eq!(
        metadata_document_twitter_card_description.to_string().as_str().trim(),
