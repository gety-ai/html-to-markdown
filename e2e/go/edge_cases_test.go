...
// alef:hash:666844feb7aa15c3ac36732d460f34beaf30ef08e070bf0f490555eedb18a00d
...
func Test_EncodingCjkCharacters(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `中文内容`) {
			t.Errorf("expected to contain %s", `中文内容`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `日本語テキスト`) {
			t.Errorf("expected to contain %s", `日本語テキスト`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `한국어 텍스트`) {
...
func Test_MalformedDeeplyNestedElements(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `Deeply nested content`) {
...
func Test_MalformedOverlappingTags(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `bold and italic`) {
...
func Test_MalformedUnclosedParagraph(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `This paragraph is never closed`) {
...
func Test_VisitorCustomElementWithNesting(t *testing.T) {
	...
	result, err := htmd.ConvertWithVisitor(`<div><custom-widget data-value="123"><p>Widget content here</p><span>With nested elements</span></custom-widget></div>`, nil, visitor)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	var content string
	...
	if !strings.Contains(fmt.Sprint(content), `[CUSTOM WIDGET]`) {
		t.Errorf("expected to contain %s, got %v", `[CUSTOM WIDGET]`, content)
	}
	if strings.Contains(fmt.Sprint(content), `Widget content here`) {
...
func Test_VisitorDeeplyNestedSkip(t *testing.T) {
	...
	result, err := htmd.ConvertWithVisitor(`<div><p>Outer <em>emphasis <strong>with bold <mark>and highlight</mark></strong></em> text</p></div>`, nil, visitor)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	var content string
	...
	if !strings.Contains(fmt.Sprint(content), `Outer`) {
		t.Errorf("expected to contain %s, got %v", `Outer`, content)
	}
	if !strings.Contains(fmt.Sprint(content), `text`) {
		t.Errorf("expected to contain %s, got %v", `text`, content)
	}
	if strings.Contains(fmt.Sprint(content), `highlight`) {
...
func Test_VisitorElementEndModification(t *testing.T) {
	...
	result, err := htmd.ConvertWithVisitor(`<blockquote><p>Original quote</p></blockquote>`, nil, visitor)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.Content == nil {
...
func Test_VisitorElementStartSkipEntireSubtree(t *testing.T) {
	...
	result, err := htmd.ConvertWithVisitor(`<div><h1>Title</h1><p>Content</p></div>`, nil, visitor)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.Content == nil {
...
func Test_VisitorUnknownTagPreservation(t *testing.T) {
	...
	result, err := htmd.ConvertWithVisitor(`<article><p>Article text</p><x-custom>Custom element with content</x-custom><p>More article text</p></article>`, nil, visitor)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	var content string
	...
	if !strings.Contains(fmt.Sprint(content), `More article text`) {
		t.Errorf("expected to contain %s, got %v", `More article text`, content)
	}
	if !strings.Contains(fmt.Sprint(content), `<x-custom>`) {
...
func Test_XssOnclickHandlerRemoved(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `Click me`) {
...
func Test_XssScriptTagStripped(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `Safe content`) {
			t.Errorf("expected to contain %s", `Safe content`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `More safe content`) {
...
func Test_XssSvgNestedScriptStripped(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `Before SVG`) {
			t.Errorf("expected to contain %s", `Before SVG`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `After SVG`) {
