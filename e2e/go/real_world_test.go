...
// alef:hash:665825dd2dcbb7aab3506f2ffd8070b5aa7a391e0974e21ce75accac9c129892
...
func Test_RealWorldBlogPost(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `# Getting Started with Rust`) {
			t.Errorf("expected to contain %s", `# Getting Started with Rust`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `## Installation`) {
			t.Errorf("expected to contain %s", `## Installation`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `## Hello World`) {
			t.Errorf("expected to contain %s", `## Hello World`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `## Key Concepts`) {
			t.Errorf("expected to contain %s", `## Key Concepts`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `cargo run`) {
			t.Errorf("expected to contain %s", `cargo run`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `[Mozilla](https://www.mozilla.org)`) {
			t.Errorf("expected to contain %s", `[Mozilla](https://www.mozilla.org)`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `- Ownership and borrowing`) {
...
assert_eq!(markdown, "# Hello");</code></pre><h2>ConversionOptions</h2><p>Configure conversion behavior using the builder pattern:</p><pre><code class="language-rust">let options = ConversionOptions::builder()
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `# API Reference`) {
			t.Errorf("expected to contain %s", `# API Reference`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `## convert_html`) {
			t.Errorf("expected to contain %s", `## convert_html`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `### Parameters`) {
			t.Errorf("expected to contain %s", `### Parameters`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `### Returns`) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `## ConversionOptions`) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `thread-safe`) {
...
func Test_RealWorldProductPage(t *testing.T) {
	...
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `# Wireless Keyboard Pro`) {
			t.Errorf("expected to contain %s", `# Wireless Keyboard Pro`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `![Wireless Keyboard Pro](https://example.com/keyboard.jpg)`) {
			t.Errorf("expected to contain %s", `![Wireless Keyboard Pro](https://example.com/keyboard.jpg)`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `## Specifications`) {
			t.Errorf("expected to contain %s", `## Specifications`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `Battery Life`) {
			t.Errorf("expected to contain %s", `Battery Life`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `12 months`) {
			t.Errorf("expected to contain %s", `12 months`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `Bluetooth 5.0`) {
			t.Errorf("expected to contain %s", `Bluetooth 5.0`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `## What's in the Box`) {
			t.Errorf("expected to contain %s", `## What's in the Box`)
		}
	}
	if result.Content != nil {
		if !strings.Contains(fmt.Sprint(*result.Content), `USB-C charging cable`) {
