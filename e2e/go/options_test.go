...
func Test_OptionsCaptureSvgFalse(t *testing.T) {
	...
	if !strings.Contains(string(content), `Below SVG`) {
...
func Test_OptionsInferDimensionsFalse(t *testing.T) {
	...
	if !strings.Contains(string(content), `No dims`) {
...
func Test_OptionsInferDimensionsTrue(t *testing.T) {
	...
	if !strings.Contains(string(content), `With dims`) {
...
func Test_OptionsMaxImageSizeTinyLimitSafe(t *testing.T) {
	...
	if !strings.Contains(string(content), `Tiny limit`) {
...
func Test_OptionsPreprocessingEnabledFalseSkipsCleanup(t *testing.T) {
	...
	if result.Content != nil {
	if !strings.Contains(string(*result.Content), `NavSection`) {
		t.Errorf("expected to contain %s", `NavSection`)
	}
	}
	if result.Content != nil {
	if !strings.Contains(string(*result.Content), `Paragraph`) {
...
func Test_OptionsPreprocessingRemoveNavigationFalseKeepsNav(t *testing.T) {
	...
	if result.Content != nil {
	if !strings.Contains(string(*result.Content), `SiteMenu`) {
		t.Errorf("expected to contain %s", `SiteMenu`)
	}
	}
	if result.Content != nil {
	if !strings.Contains(string(*result.Content), `MainContent`) {
		t.Errorf("expected to contain %s", `MainContent`)
	}
	}
	if result.Content != nil {
	if !strings.Contains(string(*result.Content), `SidebarText`) {
...
func Test_OptionsUrlEscapeStyleAngleDefault(t *testing.T) {
	// Default angle style wraps URLs containing spaces in angle brackets
	var options htmd.ConversionOptions
	if err := json.Unmarshal([]byte(`{"url_escape_style":"angle"}`), &options); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	...
	if !strings.Contains(string(content), `</file (1).pdf>`) {
		t.Errorf("expected to contain %s, got %v", `</file (1).pdf>`, content)
	}
}

func Test_OptionsUrlEscapeStylePercentAngleBracketsInUrl(t *testing.T) {
	// Percent style encodes angle brackets in link URLs from issue 392
	var options htmd.ConversionOptions
	if err := json.Unmarshal([]byte(`{"url_escape_style":"percent"}`), &options); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	...
	if !strings.Contains(string(content), `/file%20%281%29%20%3Cdraft%3E.pdf`) {
		t.Errorf("expected to contain %s, got %v", `/file%20%281%29%20%3Cdraft%3E.pdf`, content)
	}
}

func Test_OptionsUrlEscapeStylePercentImage(t *testing.T) {
	...
	if !strings.Contains(string(content), `/img%20%281%29%20%3Cdraft%3E.png`) {
		t.Errorf("expected to contain %s, got %v", `/img%20%281%29%20%3Cdraft%3E.png`, content)
	}
}

func Test_OptionsUrlEscapeStylePercentLink(t *testing.T) {
	...
	if !strings.Contains(string(content), `/file%20%281%29.pdf`) {
		t.Errorf("expected to contain %s, got %v", `/file%20%281%29.pdf`, content)
	}
	if strings.Contains(string(content), `</`) {
		t.Errorf("expected NOT to contain %s, got %v", `</`, content)
	}
