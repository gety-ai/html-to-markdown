...
// alef:hash:bc46aafd2f0a95ff66959b3b66b29bfbaf47100c5eb3075725796c617b825fa8
...
final class EdgeCasesTest extends TestCase
{
    ...
    {        $result = HtmlToMarkdown::convert("<p>中文内容</p><p>日本語テキスト</p><p>한국어 텍스트</p>");
            ...
            $this->assertStringContainsString("中文内容", $result->content);
    $this->assertStringContainsString("日本語テキスト", $result->content);
    $this->assertStringContainsString("한국어 텍스트", $result->content);
    ...
    {        $result = HtmlToMarkdown::convert("<div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><p>Deeply nested content</p></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div>");
            $this->assertNotEmpty($result->content);
            $this->assertStringContainsString("Deeply nested content", $result->content);
    ...
    {        $result = HtmlToMarkdown::convert("<p><b><i>bold and italic</b></i></p>");
            $this->assertNotEmpty($result->content);
            $this->assertStringContainsString("bold and italic", $result->content);
    ...
    {        $result = HtmlToMarkdown::convert("<p>This paragraph is never closed");
            $this->assertNotEmpty($result->content);
            $this->assertStringContainsString("This paragraph is never closed", $result->content);
    ...
    {        $visitor = new class {
        ...
        $result = HtmlToMarkdown::convert("<div><custom-widget data-value=\"123\"><p>Widget content here</p><span>With nested elements</span></custom-widget></div>", $options);
            $this->assertStringContainsString("[CUSTOM WIDGET]", $result->content);
            $this->assertStringNotContainsString("Widget content here", $result->content);
    ...
    {        $visitor = new class {
        ...
        $result = HtmlToMarkdown::convert("<div><p>Outer <em>emphasis <strong>with bold <mark>and highlight</mark></strong></em> text</p></div>", $options);
            ...
            $this->assertStringNotContainsString("highlight", $result->content);
    ...
    {        $visitor = new class {
        ...
        $result = HtmlToMarkdown::convert("<article><p>Article text</p><x-custom>Custom element with content</x-custom><p>More article text</p></article>", $options);
            ...
            $this->assertStringContainsString("More article text", $result->content);
            $this->assertStringContainsString("<x-custom>", $result->content);
    ...
    $this->assertStringNotContainsString("steal_data", $result->content);
    ...
    {        $result = HtmlToMarkdown::convert("<p>Safe content.</p><script>alert('xss')</script><p>More safe content.</p>");
            $this->assertNotEmpty($result->content);
            $this->assertStringContainsString("Safe content", $result->content);
    $this->assertStringContainsString("More safe content", $result->content);
            $this->assertStringNotContainsString("<script>", $result->content);
    $this->assertStringNotContainsString("alert(", $result->content);
    $this->assertStringNotContainsString("xss", $result->content);
    ...
    {        $result = HtmlToMarkdown::convert("<p>Before SVG.</p><svg xmlns=\"http://www.w3.org/2000/svg\"><script>alert('svg-xss')</script><text>SVG text</text></svg><p>After SVG.</p>");
            $this->assertNotEmpty($result->content);
            $this->assertStringContainsString("Before SVG", $result->content);
    $this->assertStringContainsString("After SVG", $result->content);
            $this->assertStringNotContainsString("<script>", $result->content);
    $this->assertStringNotContainsString("alert(", $result->content);
    $this->assertStringNotContainsString("svg-xss", $result->content);
