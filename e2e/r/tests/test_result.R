...
# alef:hash:782be16d2a01680aaf6bda5c49b00f9ff6a3ed43f7b1df34aabdba0e40d6b324
...
test_that("result_tables_empty_when_no_tables: Result tables array is empty when input has no tables", {
  result <- convert(html = "<p>No tables here</p>", options = list("include_document_structure" = TRUE))
...
test_that("result_tables_multiple: Multiple tables each appear in the tables array", {
  result <- convert(html = "<table><tr><th>A</th></tr><tr><td>1</td></tr></table><p>Between</p><table><tr><th>B</th></tr><tr><td>2</td></tr></table>", options = list("include_document_structure" = TRUE))
  expect_true(length(result$tables) >= 2)
})

test_that("result_tables_simple: Simple table populates the tables array in result", {
  result <- convert(html = "<table><thead><tr><th>Name</th><th>Age</th></tr></thead><tbody><tr><td>Alice</td><td>30</td></tr></tbody></table>", options = list("include_document_structure" = TRUE))
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("result_tables_without_structure_flag: Tables array is empty when includeDocumentStructure is false", {
  result <- convert(html = "<table><tr><th>X</th></tr><tr><td>Y</td></tr></table>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
  expect_equal(length(result$tables), 0)
})

test_that("result_warnings_empty_for_clean_input: Warnings array is empty for well-formed HTML without problematic content", {
  result <- convert(html = "<h1>Title</h1><p>Clean content with <a href='https://example.com'>a link</a>.</p>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("result_warnings_empty_for_complex_input: Warnings array is empty for complex but valid HTML", {
  result <- convert(html = "<article><h1>Article</h1><p>Paragraph with <strong>bold</strong> and <em>italic</em>.</p><table><tr><th>Col</th></tr><tr><td>Val</td></tr></table><ul><li>Item 1</li><li>Item 2</li></ul></article>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
  expect_equal(length(result$warnings), 0)
})

test_that("result_warnings_empty_for_malformed_html: Warnings array is empty even for malformed HTML (parser is lenient)", {
  result <- convert(html = "<p>Unclosed paragraph<div>Mixed nesting</p></div>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
  expect_equal(length(result$warnings), 0)
})
