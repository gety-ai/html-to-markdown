...
# alef:hash:12652f43768285e2b85baec211142083703a8c3309da432343fea253605d33b0
...
test_that("visitor_custom_element_with_nesting: Visitor handles custom elements with nested content", {
  ...
  result <- convert(html = "<div><custom-widget data-value=\"123\"><p>Widget content here</p><span>With nested elements</span></custom-widget></div>", options = ConversionOptions$default(), options = list(visitor = visitor))
...
test_that("visitor_deeply_nested_skip: Visitor skips deeply nested elements", {
  ...
  result <- convert(html = "<div><p>Outer <em>emphasis <strong>with bold <mark>and highlight</mark></strong></em> text</p></div>", options = ConversionOptions$default(), options = list(visitor = visitor))
...
test_that("visitor_element_end_modification: Visitor modifies element at end after children processed", {
  ...
  result <- convert(html = "<blockquote><p>Original quote</p></blockquote>", options = ConversionOptions$default(), options = list(visitor = visitor))
  expect_true(if (is.character(result$content)) length(result$content) > 0 && any(nchar(result$content) > 0) else length(result$content) > 0)
})
...
test_that("visitor_element_start_skip_entire_subtree: Visitor skips at element_start level removes entire subtree", {
  ...
  result <- convert(html = "<div><h1>Title</h1><p>Content</p></div>", options = ConversionOptions$default(), options = list(visitor = visitor))
...
test_that("visitor_unknown_tag_preservation: Visitor preserves unknown HTML tags as raw HTML", {
  ...
  result <- convert(html = "<article><p>Article text</p><x-custom>Custom element with content</x-custom><p>More article text</p></article>", options = ConversionOptions$default(), options = list(visitor = visitor))
