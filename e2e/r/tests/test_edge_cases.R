...
# alef:hash:9ef97efe36e0c7ee1252197c74622a3b0da1e7ae2b58f10bcab57688ec2c35b1
...
test_that("empty_html: Empty HTML document", {
  result <- convert(html = "<html><head></head><body></body></html>", options = NULL)
  expect_equal(trimws(result$content), "")
})

test_that("encoding_cjk_characters: CJK (Chinese, Japanese, Korean) characters are preserved", {
  result <- convert(html = "<p>中文内容</p><p>日本語テキスト</p><p>한국어 텍스트</p>", options = NULL)
...
test_that("encoding_html_entities: Common HTML entities are decoded in output", {
  result <- convert(html = "<p>&amp; &lt; &gt; &nbsp; &quot; &apos;</p>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("encoding_named_entities: Named HTML entities like &mdash; and &hellip; are decoded", {
  result <- convert(html = "<p>Em dash&mdash;used for parenthetical remarks&mdash;is common. Ellipsis&hellip; indicates omission. Non-breaking&nbsp;space.</p>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
  expect_true(any(grepl("—", result$content, fixed = TRUE)))
  expect_true(any(grepl("…", result$content, fixed = TRUE)))
})
...
test_that("encoding_numeric_entities: Numeric HTML entities (decimal and hex) are decoded", {
  result <- convert(html = "<p>Copyright: &#169; Trade: &#174; Euro: &#8364; Hex: &#x00A9;</p>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
  expect_true(any(grepl("©", result$content, fixed = TRUE)))
  expect_true(any(grepl("®", result$content, fixed = TRUE)))
  expect_true(any(grepl("€", result$content, fixed = TRUE)))
...
test_that("encoding_unicode_emoji: Emoji and Unicode characters are preserved", {
  result <- convert(html = "<p>Hello 🌍 World 🚀</p><p>Stars: ⭐ ✨</p>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
  expect_true(any(grepl("🌍", result$content, fixed = TRUE)))
  expect_true(any(grepl("🚀", result$content, fixed = TRUE)))
  expect_true(any(grepl("⭐", result$content, fixed = TRUE)))
...
test_that("html_comments_only: Document containing only HTML comments produces empty output", {
  result <- convert(html = "<!-- This is a comment --><!-- Another comment -->", options = NULL)
  expect_equal(trimws(result$content), "")
})
...
test_that("malformed_deeply_nested_elements: Deeply nested elements (100 levels) are handled without stack overflow", {
  result <- convert(html = "<div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><p>Deeply nested content</p></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("malformed_missing_block_closing_tags: Missing closing tags on block elements are auto-closed by parser", {
  result <- convert(html = "<div><h1>Title<p>First paragraph<p>Second paragraph</div>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("malformed_overlapping_tags: Overlapping bold/italic tags are recovered by the HTML parser without panic", {
  result <- convert(html = "<p><b><i>bold and italic</b></i></p>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("malformed_unclosed_paragraph: Unclosed <p> tag is recovered gracefully and content is preserved", {
  result <- convert(html = "<p>This paragraph is never closed", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("script_tags_only: Document with only script tags produces empty output (scripts are stripped)", {
  result <- convert(html = "<html><head><script>alert('xss')</script></head><body><script>document.write('hello')</script></body></html>", options = NULL)
  expect_equal(trimws(result$content), "")
})

test_that("style_tags_only: Document with only style tags produces empty output (styles are stripped)", {
  result <- convert(html = "<html><head><style>body { color: red; }</style></head><body><style>.foo { margin: 0; }</style></body></html>", options = NULL)
  expect_equal(trimws(result$content), "")
})
...
test_that("xss_onclick_handler_removed: onclick and other on* event handlers are removed from elements", {
  result <- convert(html = "<p><a href=\"https://example.com\" onclick=\"alert('xss')\">Click me</a></p><button onmouseover=\"steal_data()\">Hover me</button>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("xss_script_tag_stripped: Script tag content is stripped and does not appear in output", {
  result <- convert(html = "<p>Safe content.</p><script>alert('xss')</script><p>More safe content.</p>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
...
test_that("xss_svg_nested_script_stripped: Script tags nested inside SVG are stripped", {
  result <- convert(html = "<p>Before SVG.</p><svg xmlns=\"http://www.w3.org/2000/svg\"><script>alert('svg-xss')</script><text>SVG text</text></svg><p>After SVG.</p>", options = NULL)
  expect_true(if (is.character(result$content)) nchar(result$content) > 0 else length(result$content) > 0)
