...
describe("result", () => {
	...
	it("result_tables_multiple: Multiple tables each appear in the tables array", () => {
		const result = convert("<table><tr><th>A</th></tr><tr><td>1</td></tr></table><p>Between</p><table><tr><th>B</th></tr><tr><td>2</td></tr></table>", { includeDocumentStructure: true } as ConversionOptions);
	...
	it("result_tables_simple: Simple table populates the tables array in result", () => {
		const result = convert("<table><thead><tr><th>Name</th><th>Age</th></tr></thead><tbody><tr><td>Alice</td><td>30</td></tr></tbody></table>", { includeDocumentStructure: true } as ConversionOptions);
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
	...
	it("result_warning_kind_image_extraction_failed: A malformed data URI with extract_images enabled produces an ImageExtractionFailed warning", () => {
		const result = convert("<p>Text<img src=\"data:BADMIME\" alt=\"broken\">end</p>", { extractImages: true } as ConversionOptions);
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
