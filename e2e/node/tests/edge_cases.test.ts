...
// alef:hash:54673e4eb5ff4a57e6a1747e4706fc7bf177dd5dcd68fbb590ce3e4d5a476e4f
...
describe('edge-cases', () => {
  it('empty_html: Empty HTML document', () => {
    const result = convert("<html><head></head><body></body></html>", {} as unknown as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("");
  });

  it('encoding_cjk_characters: CJK (Chinese, Japanese, Korean) characters are preserved', () => {
    const result = convert("<p>中文内容</p><p>日本語テキスト</p><p>한국어 텍스트</p>", {} as unknown as ConversionOptions);
  ...
  it('encoding_named_entities: Named HTML entities like &mdash; and &hellip; are decoded', () => {
    const result = convert("<p>Em dash&mdash;used for parenthetical remarks&mdash;is common. Ellipsis&hellip; indicates omission. Non-breaking&nbsp;space.</p>", {} as unknown as ConversionOptions);
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
    expect(result.content).toContain("—");
    expect(result.content).toContain("…");
  });
  ...
  it('encoding_numeric_entities: Numeric HTML entities (decimal and hex) are decoded', () => {
    const result = convert("<p>Copyright: &#169; Trade: &#174; Euro: &#8364; Hex: &#x00A9;</p>", {} as unknown as ConversionOptions);
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
    expect(result.content).toContain("©");
    expect(result.content).toContain("®");
    expect(result.content).toContain("€");
  ...
  it('encoding_unicode_emoji: Emoji and Unicode characters are preserved', () => {
    const result = convert("<p>Hello 🌍 World 🚀</p><p>Stars: ⭐ ✨</p>", {} as unknown as ConversionOptions);
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
    expect(result.content).toContain("🌍");
    expect(result.content).toContain("🚀");
    expect(result.content).toContain("⭐");
  ...
  it('html_comments_only: Document containing only HTML comments produces empty output', () => {
    const result = convert("<!-- This is a comment --><!-- Another comment -->", {} as unknown as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("");
  });
  ...
  it('malformed_missing_block_closing_tags: Missing closing tags on block elements are auto-closed by parser', () => {
    const result = convert("<div><h1>Title<p>First paragraph<p>Second paragraph</div>", {} as unknown as ConversionOptions);
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
  ...
  it('malformed_overlapping_tags: Overlapping bold/italic tags are recovered by the HTML parser without panic', () => {
    const result = convert("<p><b><i>bold and italic</b></i></p>", {} as unknown as ConversionOptions);
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
  ...
  it('script_tags_only: Document with only script tags produces empty output (scripts are stripped)', () => {
    const result = convert("<html><head><script>alert('xss')</script></head><body><script>document.write('hello')</script></body></html>", {} as unknown as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("");
  });

  it('style_tags_only: Document with only style tags produces empty output (styles are stripped)', () => {
    const result = convert("<html><head><style>body { color: red; }</style></head><body><style>.foo { margin: 0; }</style></body></html>", {} as unknown as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("");
  });
  ...
  it('visitor_custom_element_with_nesting: Visitor handles custom elements with nested content', () => {
    ...
    const result = convert("<div><custom-widget data-value=\"123\"><p>Widget content here</p><span>With nested elements</span></custom-widget></div>", {} as unknown as ConversionOptions, { visitor: _testVisitor });
  ...
  it('visitor_deeply_nested_skip: Visitor skips deeply nested elements', () => {
    ...
    const result = convert("<div><p>Outer <em>emphasis <strong>with bold <mark>and highlight</mark></strong></em> text</p></div>", {} as unknown as ConversionOptions, { visitor: _testVisitor });
  ...
  it('visitor_element_end_modification: Visitor modifies element at end after children processed', () => {
    ...
    const result = convert("<blockquote><p>Original quote</p></blockquote>", {} as unknown as ConversionOptions, { visitor: _testVisitor });
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
  });
  ...
  it('visitor_element_start_skip_entire_subtree: Visitor skips at element_start level removes entire subtree', () => {
    ...
    const result = convert("<div><h1>Title</h1><p>Content</p></div>", {} as unknown as ConversionOptions, { visitor: _testVisitor });
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
  });
  ...
  it('visitor_unknown_tag_preservation: Visitor preserves unknown HTML tags as raw HTML', () => {
    ...
    const result = convert("<article><p>Article text</p><x-custom>Custom element with content</x-custom><p>More article text</p></article>", {} as unknown as ConversionOptions, { visitor: _testVisitor });
  ...
  it('xss_onclick_handler_removed: onclick and other on* event handlers are removed from elements', () => {
    const result = convert("<p><a href=\"https://example.com\" onclick=\"alert('xss')\">Click me</a></p><button onmouseover=\"steal_data()\">Hover me</button>", {} as unknown as ConversionOptions);
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
  ...
  it('xss_script_tag_stripped: Script tag content is stripped and does not appear in output', () => {
    const result = convert("<p>Safe content.</p><script>alert('xss')</script><p>More safe content.</p>", {} as unknown as ConversionOptions);
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
  ...
  it('xss_svg_nested_script_stripped: Script tags nested inside SVG are stripped', () => {
    const result = convert("<p>Before SVG.</p><svg xmlns=\"http://www.w3.org/2000/svg\"><script>alert('svg-xss')</script><text>SVG text</text></svg><p>After SVG.</p>", {} as unknown as ConversionOptions);
    expect((() => { const v = (result.content as unknown) ?? ''; return typeof v === 'string' || Array.isArray(v) ? (v as { length: number }).length : Object.keys(v as object).length; })()).toBeGreaterThan(0);
